...
void tryAllRoutes(int *route, int **distanceMatrix, int start, int n, int *baseRoute, int *iterationsLeft)
{
    if (*iterationsLeft <= 0) {
        return;
    }
    ...
    for (int i = start; i < n; i++)
    {
        ...
        tryAllRoutes(route, distanceMatrix, start + 1, n, baseRoute, iterationsLeft);
